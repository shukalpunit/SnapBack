...
export interface ProductivitySlot {
  /** Hour of day (0–23) */
  hour: number;
  /** Average deep work minutes in this hour across analyzed days */
  avgDeepWorkMinutes: number;
  /** Productivity score (0–1, normalized) */
  score: number;
}
...
export interface TimeBlockSuggestion {
  ...
  /** Suggested start time (Unix ms) */
  startTime: number;
  /** Suggested end time (Unix ms) */
  endTime: number;
  /** The productivity score of this slot */
  slotScore: number;
...
export class TimeBlockScheduler {
   ...
   */
  analyzeProductivityByHour(refDate?: Date): ProductivitySlot[] {
    ...
    for (let d = 1; d <= ANALYSIS_WINDOW_DAYS; d++) {
      ...
      for (const seg of segments) {
        if (seg.classification !== 'deep_work') continue;
    ...
    const avgMinutes = hourTotals.map((total) =>
      daysAnalyzed > 0 ? total / daysAnalyzed : 0
    );
   ...
   */
  getPeakHours(refDate?: Date, minScore = 0.3): ProductivitySlot[] {
  ...
  generateSuggestions(
    ...
    for (const task of incompleteTasks) {
      if (suggestions.length >= peakHours.length) break;
      ...
      for (const slot of peakHours) {
        ...
        const hasConflict = existingEvents.some((evt) =>
          overlapMs(evt.startTime, evt.endTime, blockStart, blockEnd) > 0
        );
   ...
   */
  toCalendarBlocks(suggestions: TimeBlockSuggestion[]): CalendarTimeBlock[] {
