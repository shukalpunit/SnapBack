...
export interface HeatMapCell {
  cellIndex: number;          // 0–95
  startTime: number;          // Unix ms — start of the 15-min block
  endTime: number;            // Unix ms — end of the 15-min block
  classification: Classification | null; // null if no data in this block
  hasData: boolean;
}
...
export class HeatMapService {
   ...
   */
  getHeatMapCells(date: string): HeatMapCell[] {
      ...
      for (const seg of segments) {
        ...
        if (overlap > 0) {
          ...
          switch (seg.classification) {
            case 'deep_work': deepMs += overlap; break;
            case 'shallow_work': shallowMs += overlap; break;
            case 'distraction_loop': distractionMs += overlap; break;
          }
        }
      }
   ...
   */
  getTooltipData(cellIndex: number, date: string): TooltipData | null {
    if (cellIndex < 0 || cellIndex >= CELLS_PER_DAY) return null;
    ...
    if (appDurations.size === 0) return null;

    // Find dominant app
    let bestApp = '';
    let bestDuration = 0;
    let bestClassification: Classification = 'shallow_work';
